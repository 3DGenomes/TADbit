...
.. code:: python
    ...
    models = exp.model_region(100, 200, n_models=5000, n_keep=1000, n_cpus=8, keep_all=True, config=optpar)
    print models


.. parsed-literal::

    /usr/local/lib/python2.7/dist-packages/pytadbit/experiment.py:428: UserWarning: WARNING: normalizing according to visibility method
...
.. code:: python
    ...
    models.cluster_models(fact=0.95, dcutoff=1500)
...
.. code:: python
    ...
    models.contact_map(models=range(5,10), cutoff=1500, savedata="contact.txt")
...
.. code:: python
    ...
    models.view_model(models=[0], tool='chimera_nogui', savefig='/tmp/image_model_1.png')

.. code:: python

    # Generate the video using Chimera in batch mode. That takes SIGNIFICANT time, wait a bit before running next command.
    # You can check in your home directory whether this has finished.
    models.view_model(models=[0], tool='chimera_nogui', savefig='/tmp/image_model_1.webm')
