...
from numpy import linalg, array, log2, copy, minimum, newaxis, median
...
def main():
    ...
    # scale distances
    meand = median([D[i][i+1] for i in xrange(N-1)])
    factor = 5 * float(scale) / meand

    # get the square of D for performence
    D2 = [[(factor*D[i][j])**2 for j in xrange(N)] for i in xrange(N)]
    ...
    if show:
        ...
        plt.show()
    if cmm:
        write_cmm(x, y, z, fnam.split('.')[0] + '.cmm', radius=30)
        write_xyz(x, y, z, fnam.split('.')[0] + '.xyz')
...
def get_options():
    ...
    parser.add_argument('--scale', dest='scale',
                        default=200, help='''[%(default)s] Average distance (nm)
                        beetween two particles.''')
