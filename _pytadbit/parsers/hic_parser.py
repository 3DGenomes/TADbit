...
def optimal_reader(f, resolution=1):
    """
    ...
    # apply_async the pacth
    HiC_data.__setitem__ = fast_setitem

    hic = HiC_data(((i + j * ncol, num(v))
                    for i, line in enumerate(f)
                    for j, v in enumerate(line[2 + i:])  # lose symmetry check
                    if num(v)), size=ncol, masked=masked,
                   dict_sec=sections, chromosomes=chromosomes,
                   resolution=resolution, symmetricized=True)
