...
def _filter_same_frag(fnam, max_molecule_length):
    # t0 = time()
    masked = {1 : {'name': 'self-circle'       , 'reads': set()}, 
              2 : {'name': 'dangling-end'      , 'reads': set()},
              3 : {'name': 'error'             , 'reads': set()},
...
def _filter_duplicates(fnam):
    # t0 = time()
    total = 0
    masked = {9 : {'name': 'duplicated'        , 'reads': set()}}
    uniq_check = set()
    fhandler = open(fnam)
    line = fhandler.next()
    while line.startswith('#'):
...
def _filter_from_res(fnam, max_frag_size, min_dist_to_re,
                     re_proximity, min_frag_size):
    # t0 = time()
    masked = {5 : {'name': 'too close from RES', 'reads': set()},
              6 : {'name': 'too short'         , 'reads': set()},
              7 : {'name': 'too large'         , 'reads': set()},
              10: {'name': 'random breaks'     , 'reads': set()}}
    fhandler = open(fnam)
...
def _filter_over_represented(fnam, over_represented):
    ...
    # use cut-1 because it represents the length of the list
    cut = sorted([frag_count[crm] for crm in frag_count])[cut - 1]
    masked = {8 : {'name': 'over-represented'  , 'reads': set()}}
    fhandler = open(fnam)
    line = fhandler.next()
    while line.startswith('#'):
        line = fhandler.next()
